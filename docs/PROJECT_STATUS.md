...
### Add Animated HTTP Error States and Session-Expiry Recovery
- **Date/Time**: 2026-07-14T17:53:31+07:00
- **Implementation**: Adapted the supplied v0 404 physics animation into reusable Vela Wear React/Tailwind components without copying scaffold dependencies or placeholder assets. Added responsive route-level 404, standalone 403, root error, and global 5xx experiences; hardened the animation with a bounded drop count, resize handling, idle/hidden-tab pausing, and reduced-motion support. Management resource tables now retain raw Axios status information and render the same compact animated experience for 403, 404, and all 5xx responses across Users, Roles, Permissions, Products, Orders, Coupons, Categories, Brands, Colors, and Sizes. Active-session 401 responses still attempt one token refresh, then clear the access token and return to sign-in with a validated internal redirect that works for password and Google OAuth sign-in.
- **Verification**: Scoped ESLint passes with zero warnings, `pnpm exec tsc --noEmit --pretty false` passes, and `pnpm build` passes with all 63 production pages including `/unauthorized` and the root not-found/error boundaries. Browser QA verified the desktop and 375x812 animated 404 layouts, the interactive code-drop behavior, the standalone 403 content, exact mobile fit without horizontal overflow, and zero console warnings/errors. Full `pnpm lint` remains blocked by 10 pre-existing errors and 25 warnings in legacy/shared files such as `nav-main.tsx`, invoice/mail effects, tasks copy, the storefront header, carousel, responsive hooks, and a scratch script.
- **Known Follow-ups**: This work intentionally starts from `origin/main` as a separate change, so it may require a small rebase around shared auth and Management files if the role-aware loading PR merges first. Other non-authentication 4xx responses continue to use the existing generic inline Management alert.
