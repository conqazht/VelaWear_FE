...
| :------ | :----------- | :--------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `page`  | `number`     | `1`              | **Chỉ mục trang bắt đầu từ 1** (Trang 1 là trang đầu tiên). _Lưu ý: Backend đã cấu hình `one-indexed-parameters: true` nên Frontend tuyệt đối không gửi trang đầu là `0`._ |
...
_Tài liệu quy ước này là cơ sở duy nhất cho việc phát triển và tích hợp giao diện. Vui lòng thảo luận và cập nhật tài liệu này nếu có bất kỳ sự thay đổi nào từ phía API Backend._
