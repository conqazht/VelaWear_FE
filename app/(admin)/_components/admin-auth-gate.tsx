...
function AdminSessionLoadingFrame() {
  return (
    <div className="bg-background grid min-h-screen lg:grid-cols-[17rem_1fr]">
      <aside className="bg-sidebar hidden border-r p-4 lg:flex lg:flex-col lg:gap-6">
  ...
  if (!isAuthenticated) {
    return (
      <div className="bg-background relative flex min-h-screen items-center justify-center p-6">
        <LanguageSwitcher presentation="popover" className="absolute top-5 right-5" />
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <CardDescription>{t("admin.shell.auth.signInDescription")}</CardDescription>
          </CardHeader>
  ...
  if (!canAccessManagement(user)) {
    ...
    const currentRoleDescription =
      roleNames.length > 0
        ? t(
            roleNames.length === 1
              ? "admin.shell.auth.currentRole"
              : "admin.shell.auth.currentRoles",
            {
