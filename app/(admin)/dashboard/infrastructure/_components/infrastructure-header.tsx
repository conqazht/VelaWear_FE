      ...
      <div className="flex flex-col gap-2">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex min-w-0 flex-col gap-1">
            <h1 className="text-2xl leading-tight font-medium tracking-tight sm:text-3xl sm:leading-none">
              {t("admin.infrastructure.title")}
            </h1>
            <p className="text-muted-foreground text-sm">{t("admin.infrastructure.description")}</p>
          </div>

          <div className="flex w-full items-center justify-between gap-2 sm:w-auto sm:justify-end">
            <span className="text-muted-foreground text-sm whitespace-nowrap">
              {t("admin.infrastructure.updated", { seconds: formatNumber(30, locale) })}
            </span>
            <div className="flex items-center gap-2">
              <Button
                variant="outline"
                size="icon-sm"
                aria-label={t("admin.infrastructure.refresh")}
              >
                <RefreshCw />
              </Button>
              <Button
                variant="outline"
                size="icon-sm"
                aria-label={t("admin.infrastructure.settings")}
              >
        ...
        <div className="flex flex-wrap gap-2">
          <Badge variant="outline" className="h-auto gap-1 rounded-sm px-1.5 py-0.5">
            <Container />
            {t("admin.infrastructure.projects", { count: formatNumber(6, locale) })}
          </Badge>
          <Badge variant="outline" className="h-auto gap-1 rounded-sm px-1.5 py-0.5">
            <Box />
          ...
          <Badge variant="outline" className="h-auto gap-1 rounded-sm px-1.5 py-0.5">
            ...
            {t("admin.infrastructure.globalUptime", {
              uptime: formatNumber(99.93, locale, { maximumFractionDigits: 2 }),
            })}
          </Badge>
        </div>
      </div>
