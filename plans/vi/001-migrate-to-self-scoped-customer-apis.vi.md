...
Mở rộng `e2e/fixtures/fullstack.ts` bằng primary/secondary account fixture chỉ đọc từ `E2E_USER_EMAIL`/`E2E_USER_PASSWORD` và `E2E_SECOND_USER_EMAIL`/`E2E_SECOND_USER_PASSWORD`; không copy credential value vào plan/log. Tạo `e2e/fullstack/customer-self-scope.spec.ts` có exact title `two accounts cannot cross self-service ownership` và `{ tag: "@fullstack" }`. Qua APIRequestContext, login hai configured disposable account, lấy A order/code/address từ self lists, chứng minh own read thành công, rồi dùng token B request order ID/code/status-history và address ID của A, tất cả HTTP `404`. Test fail với seed-prerequisite message rõ ràng nếu configured A fixture không có order/address và không tạo persistent cross-account fixture. Component/API tests chứng minh profile update không gửi `avatar`. Cart/wishlist/reviews vẫn hoạt động.
