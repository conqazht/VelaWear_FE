...
test(
  ...
  async ({ page }) => {
    ...
    expect(payload.data.result.length, "Dữ liệu test phải có ít nhất một sản phẩm ACTIVE").toBeGreaterThan(0);

    const effectivePrices = payload.data.result.map((product) =>
      product.pricing?.effectivePrice ?? product.price,
    );
    ...
    await expect(page.getByRole("heading", { name: payload.data.result[0].name, exact: true })).toBeVisible();
