...
async function openDesktopFilters(page: Page) {
  ...
  await expect(page.getByRole("button", { name: "Danh mục" })).toHaveAttribute("aria-expanded", "true");
}
...
test("mega-menu tách parent link và leaf link thật trên desktop/mobile", { tag: "@smoke" }, async ({ page }) => {
  ...
  await expect(page).toHaveURL((url) => url.pathname === "/collection" && url.searchParams.get("sort") === "newest");
...
test("catalog khôi phục multi-filter và sort giá từ URL sau reload", { tag: "@smoke" }, async ({ page }) => {
  const requests: URL[] = [];
...
test("size guide đổi cm/in và dùng accordion plus-size", { tag: "@smoke" }, async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "cm" }).first()).toHaveAttribute("aria-pressed", "true");
...
test("PDP rút gọn 3 review và modal phân trang server-side có lightbox", { tag: "@smoke" }, async ({ page }) => {
  ...
  const longComment = "Chất liệu mềm, đường may đẹp và form mặc thoải mái trong cả ngày. ".repeat(4);
  ...
  await page.route("**/api/v1/reviews/product/101?**", async (route) => {
    ...
    const result = Array.from({ length: requestedSize === 3 ? 3 : 2 }, (_, index) => ({
      ...
      images: requestedSize === 10 && index === 0
    ...
    await route.fulfill({
      ...
      json: {
        ...
        data: {
          result,
          meta: { page: requestedPage, pageSize: requestedSize, pages: requestedSize === 3 ? 13 : 4, total: 37 },
        },
      },
    });
  });
  ...
  await expect(page.getByRole("dialog").getByRole("heading", { name: `Đánh giá về ${product.name}` })).toBeVisible();
  ...
  await page.getByRole("navigation", { name: "Phân trang đánh giá" }).getByRole("button", { name: "2", exact: true }).click();
