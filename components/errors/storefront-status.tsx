...
type StorefrontStatusActionLabel =
  { label: string; labelKey?: never } | { label?: never; labelKey: TranslationKey };
  ...
  return (
    <StorefrontStatusView
      ...
      copy={{
        ...
        referenceText: reference ? t("errors.common.reference", { reference }) : undefined,
...
export function StorefrontStatus(props: StorefrontStatusProps) {
  ...
  return (
    <StorefrontStatusView
      ...
      secondaryAction={
        props.secondaryAction ? resolveLiteralAction(props.secondaryAction) : undefined
      }
...
function StorefrontStatusView({
  ...
  return (
    <section
      ...
      role="alert"
    >
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(rgba(181,87,58,0.055)_1px,transparent_1px),linear-gradient(90deg,rgba(181,87,58,0.055)_1px,transparent_1px)] [mask-image:linear-gradient(to_bottom,black,transparent_82%)] bg-[size:56px_56px]" />
      <div className="pointer-events-none absolute -top-40 -right-32 h-[28rem] w-[28rem] rounded-full border border-[#b5573a]/18 sm:-right-20 sm:h-[34rem] sm:w-[34rem]" />
      <div className="pointer-events-none absolute -top-24 -right-16 h-[20rem] w-[20rem] rounded-full bg-[#efe7dc]/70 sm:h-[25rem] sm:w-[25rem]" />
      <StatusCodeRain
        code={code}
        color="#b5573a"
        compact={compact}
      ...
      {!compact && !route ? (
        <div className="relative z-10 flex items-center justify-between border-b border-[#e3dccf] pb-5">
          <Link
            href="/"
            className="text-sm font-semibold tracking-[0.28em] text-[#1c1a18] uppercase transition-colors hover:text-[#b5573a]"
          >
            Vela Wear
          </Link>
          <div className="flex items-center gap-3 sm:gap-4">
            <span className="hidden text-[10px] font-medium tracking-[0.24em] text-[#55423d]/65 uppercase sm:inline">
              {copy.atelier}
            </span>
            {headerAccessory}
          </div>
      ...
      >
        <div
          className={cn(
            ...
            compact ? "justify-center sm:justify-start" : "justify-center md:justify-start",
          )}
        >
          <div
            className={cn(
          ...
          <p
            className={cn(
              "font-numeric relative leading-none font-semibold tracking-[-0.09em] text-[#b5573a]",
              compact
                ? "text-7xl sm:text-8xl lg:text-9xl"
                : "text-[7rem] sm:text-[9rem] lg:text-[12rem]",
            )}
            aria-hidden="true"
          >
            {code}
        ...
        <div className={cn(compact ? "text-center sm:text-left" : "text-center md:text-left")}>
          <p className="text-[10px] font-semibold tracking-[0.3em] text-[#b5573a] uppercase">
            {copy.eyebrow}
          </p>
          <div
            className={cn(
              "bg-[#b5573a]",
              compact ? "mx-auto my-4 h-px w-12 sm:mx-0" : "mx-auto my-5 h-px w-16 md:mx-0",
            )}
          />
          <h1
            id={titleId}
            className={cn(
              "font-serif leading-[1.12] font-medium tracking-[-0.025em] text-[#1c1a18]",
          ...
          <p
            className={cn(
              "mt-4 text-[#55423d]/80",
              compact
                ? "mx-auto max-w-lg text-sm leading-6 sm:mx-0"
                : "mx-auto max-w-xl leading-7 md:mx-0",
            )}
          >
            {description}
          </p>
          ...
          {reference ? (
            <p className="mt-3 font-mono text-[10px] tracking-[0.18em] text-[#55423d]/50 uppercase">
              {copy.referenceText}
            </p>
          ) : null}
          ...
          >
            ...
            {secondaryAction ? (
              <StorefrontStatusActionLink action={secondaryAction} primary={false} />
            ) : null}
          </div>
        </div>
      </div>
      ...
      {!compact && !route ? (
        <div className="relative z-10 flex items-center justify-between border-t border-[#e3dccf] pt-5 text-[10px] tracking-[0.2em] text-[#55423d]/55 uppercase">
