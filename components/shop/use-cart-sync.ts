...
export type LocalizedCartCopy = {
  userId: number;
  locale: Locale;
  items: CartItem[];
};

export function getPersistableCartItems(cart: CartItem[]) {
  return cart.flatMap((item) =>
    item.variantId === undefined ? [] : [{ variantId: item.variantId, quantity: item.quantity }],
  );
}

export function mapServerCartItems(serverCart: ApiCart, locale: Locale): CartItem[] {
...
export function indexCartByVariant(cart: CartItem[]) {
...
export function mergeServerCartWithLatestState(
...
export function applyLocalizedCartCopy(currentCart: CartItem[], localizedItems: CartItem[]) {
...
export interface UseCartSyncOptions {
  ...
  locale: Locale;
  storedCart: CartItem[];
  storedOwner: CartOwner;
  addToLocalCart: (product: Product, color?: string, size?: string) => void;
