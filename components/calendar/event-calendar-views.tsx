...
const getShortDayCellBottomClass = (info: DayCellInfo) => cn(!info.isNarrow && "min-h-0.5");
...
const dayRowCommonClasses: CalendarOptions = {
  ...
  listItemEventTimeClass: (info) =>
    cn(info.isNarrow ? "px-px" : "px-0.5", "order-1 whitespace-nowrap overflow-hidden shrink-1"),
  ...
  return (
    <FullCalendar
      ...
      backgroundEventTitleClass={(info) =>
        cn(
          "opacity-50 italic",
          info.isNarrow || info.isShort ? `p-1 ${xxsTextClass}` : "p-2 text-xs",
        )
      }
      ...
      blockEventClass={(info) =>
        cn(
          ...
          info.isInteractive &&
            "active:bg-[color-mix(in_oklab,var(--fc-event-color)_85%,var(--fc-event-contrast-color))]",
      ...
      rowEventTimeClass={(info) => cn(info.isNarrow ? "ps-0.5" : "ps-1")}
      rowEventTitleClass={(info) => cn(info.isNarrow ? "px-0.5" : "px-1", "font-medium")}
      ...
      columnEventTimeClass={(info) => cn(!info.isShort && (info.isNarrow ? "pt-0.5" : "pt-1"))}
      columnEventTitleClass={(info) =>
        cn(!info.isShort && (info.isNarrow ? "py-0.5" : "py-1"), "font-medium")
      }
              ...
              <span
                ...
                className={cn(
                  ...
                  textPart.type === "day" && info.isToday
                    ? [
                        "bg-chart-1/50 flex size-7 flex-row items-center justify-center rounded-full font-semibold first:-ms-1 last:-me-1",
                        info.hasNavLink &&
                          "group-hover:bg-chart-1/60 outline-ring/50 group-focus-visible:outline-3",
                      ]
            ...
            {info.textParts.map((textPart, i) => (
              <span
                ...
                className={cn(
                  ...
                  textPart.type === "day" && info.isToday
                    ? [
                        "bg-chart-1/50 flex flex-row items-center justify-center rounded-full font-semibold",
                        info.hasNavLink &&
                          "group-hover:bg-chart-1/60 outline-ring/50 group-focus-visible:outline-3",
      ...
      inlineWeekNumberClass={(info) =>
        cn(
          ...
          info.isNarrow ? `top-0.5 my-px p-0.5 ${xxsTextClass}` : "top-1 p-1 text-xs",
          info.hasNavLink && "hover:bg-foreground/5",
        )
      }
      ...
      views={{
        ...
        timeGrid: {
          ...
          dayHeaderDividerClass: (info) =>
            cn("border-b", !info.options.allDaySlot && "border-foreground/20 not-print:shadow-sm"),
          ...
          allDayHeaderInnerClass: (info) =>
            cn("m-2 text-muted-foreground", info.isNarrow ? xxsTextClass : "text-xs"),
        ...
        list: {
          ...
          listItemEventTimeClass:
            "-order-1 shrink-0 w-1/2 max-w-60 px-4 py-2 whitespace-nowrap overflow-hidden text-ellipsis text-sm",
          listItemEventTitleClass: (info) =>
            cn(
