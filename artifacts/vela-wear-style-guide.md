...
- Border radius theo phong cách Anthropic & /better-ui — 4 tầng theo ngữ cảnh: `{rounded.pill}` (9999px / `rounded-full`) cho các nút CTA chính và bộ lọc (kèm `active:scale-[0.96]`), `{rounded.2xl}` (20px) & `{rounded.xl}` (16px) cho cards & modal dialogs (chuẩn concentric), `{rounded.md}` (8px–12px) cho dropdowns & form inputs, `{rounded.sm}` (6px) cho bảng số đo và data cells.
...
**`hero-band`** — Hero full-bleed ảnh lookbook trên `{colors.surface-dark}` với overlay gradient, headline + sub-headline + CTA Pill (`rounded-full`) đặt ở góc dưới-trái hoặc giữa-trái, text `{colors.on-dark}`. Padding dọc `{spacing.section}` (88px). Phương án thay thế: 6-6 split với text trên canvas bên trái, ảnh bên phải.

**`product-card`** — Đơn vị cốt lõi của Vela Wear. Card ngoài `{rounded.xl}` **(16px)**, ảnh sản phẩm 3:4 rounded `{rounded.lg}` **(12px)**, badge ("MỚI"/"SALE") ở góc trên-trái dạng `{component.badge-pill}`, icon yêu thích góc trên-phải. Dưới ảnh: tên sản phẩm `{typography.title-md}`, giá `{typography.price}` (+ giá gốc gạch nếu sale), swatch màu nhỏ (chấm tròn 16px). Padding text dưới ảnh `{spacing.sm}` (12px). Hover: hiện shadow nhẹ.
...
**`text-input`** — Input chuẩn (tìm kiếm, email newsletter, form thanh toán). Nền `{colors.canvas}`, text `{colors.ink}`, type `{typography.body-md}`, rounded `{rounded.md}` **(8px)**, padding 12px × 16px, height 44px. Viền hairline 1px `{colors.hairline}`.

**`text-input-focused`** — Trạng thái focus. Viền chuyển sang `{colors.primary}` (terracotta), thêm outer ring 3px terracotta ở 15% alpha.

**`newsletter-band-dark`** — Băng đăng ký nhận tin trong footer hoặc giữa trang. Nền `{colors.surface-dark}`, rounded `{rounded.2xl}` **(20px)**, text `{colors.on-dark}`, chứa form dạng Floating Pill Bar (`rounded-full`) + `{component.button-primary}`.
